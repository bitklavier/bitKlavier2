...
class EnvelopeEditor : public OpenGlLineRenderer, public SynthSlider::SliderListener {
  public:
    ...
    EnvelopeEditor(const juce::String& prefix);
    ~EnvelopeEditor();
    ...
    void magnifyZoom(juce::Point<float> delta);
    void magnifyReset();
  ...
  private:
    ...
    float getBackupPhase(float phase);
    ...
    bool mouse_down_;
    juce::Point<float> last_edit_position_;

    bool animate_;
    float size_ratio_;
    float window_time_;

    float current_position_alpha_;
    float last_phase_;

    juce::Colour line_left_color_;
    juce::Colour line_right_color_;
    juce::Colour line_center_color_;
    juce::Colour fill_left_color_;
    juce::Colour fill_right_color_;
    juce::Colour background_color_;
    juce::Colour time_color_;
