...
namespace {
  juce::String formatTime(float time) {
    if (time < 1.0f) {
      int ms_value = time * bitklavier::kMsPerSec;
      return juce::String(ms_value) + "ms";
    }

    float sec_value = int(time * 10.0f) / 10.0f;
    return juce::String(sec_value) + "s";
  }
...
EnvelopeEditor::EnvelopeEditor(
    const juce::String& prefix) : OpenGlLineRenderer(kTotalPoints),
  ...
  for (int i = 0; i < kMaxTimesShown; ++i) {
    ...
    times_[i]->setJustification(juce::Justification::centredLeft);
...
void EnvelopeEditor::pickHoverPosition(juce::Point<float> position) {
  ...
  juce::Point<float> delay_point(delay_x, getHeight());
  juce::Point<float> attack_power_point((delay_x + attack_x) / 2.0f, getSliderAttackValue(0.5f));
  juce::Point<float> top_point(attack_x, 0);
  juce::Point<float> hold_point(hold_x, 0);
  juce::Point<float> decay_power_point((hold_x + decay_x) / 2.0f, getSliderDecayValue(0.5f));
  juce::Point<float> sustain_point(decay_x, sustain_y);
  juce::Point<float> release_power_point((decay_x + release_x) / 2.0f, getSliderReleaseValue(0.5f));
  juce::Point<float> release_point(release_x, getHeight());

  std::vector<juce::Point<float>> points = { top_point, sustain_point, release_point };
  if (delay_x > 0.0f)
...
void EnvelopeEditor::mouseMove(const juce::MouseEvent& e) {
  juce::Point<float> position = e.getPosition().toFloat();
  pickHoverPosition(position);
}

void EnvelopeEditor::mouseExit(const juce::MouseEvent& e) {
  delay_hover_ = false;
  attack_hover_ = false;
  hold_hover_ = false;
  sustain_hover_ = false;
...
void EnvelopeEditor::magnifyZoom(juce::Point<float> delta) {
...
float EnvelopeEditor::getBackupPhase(float phase, int index) {
  ...
  static constexpr float kTotalPhase = bitklavier::kVoiceKill - bitklavier::kVoiceOn;
  static constexpr float kDecayPoint = (bitklavier::kVoiceDecay - 1.0f * bitklavier::kVoiceOn) / kTotalPhase;
  static constexpr float kReleasePoint = (bitklavier::kVoiceOff - 1.0f * bitklavier::kVoiceOn) / kTotalPhase;
...
float EnvelopeEditor::getBackupPhase(float phase) {
//  float backup = 0.0f;
...
float EnvelopeEditor::getEnvelopeValue(float t, float power, float start, float end) {
  ...
  return start + (end - start) * powerScale(t, power);
}
...
void EnvelopeEditor::setPower(SynthSlider* slider, float power) {
  power = bitklavier::utils::clamp(power, (float)slider->getMinimum(), (float)slider->getMaximum());
...
void EnvelopeEditor::setSustainY(float y) {
  ...
  float percent = bitklavier::utils::clamp(1.0 - y / getHeight(), 0.0f, 1.0f);
...
void EnvelopeEditor::resetEnvelopeLine(int index) {
  ...
  for (int i = 0; i < kNumPointsPerSection; ++i) {
    ...
    float x = bitklavier::utils::interpolate(delay_x, attack_x, t);
  ...
  for (int i = 0; i < kNumPointsPerSection; ++i) {
    float t = (1.0f * i) / kNumPointsPerSection;
    float x = bitklavier::utils::interpolate(attack_x, hold_x, t);
  ...
  for (int i = 0; i < kNumPointsPerSection; ++i) {
    float t = (1.0f * i) / kNumPointsPerSection;
    float x = bitklavier::utils::interpolate(hold_x, decay_x, t);
  ...
  for (int i = 0; i <= kNumPointsPerSection; ++i) {
    float t = (1.0f * i) / kNumPointsPerSection;
    float x = bitklavier::utils::interpolate(decay_x, release_x, t);
...
std::pair<float, float> EnvelopeEditor::getPosition(int index) {
  float phase = 1.0f; //envelope_phase_->value()[index];

//  if (envelope_phase_->isClearValue(phase) || phase < bitklavier::kVoiceOn || phase >= bitklavier::kVoiceKill)
//    return { -1.0f, -1.0f};
...
void EnvelopeEditor::setEditingCircleBounds() {
  ...
  juce::Point<float> grab_point(-10, -10);
  if (delay_hover_)
    grab_point = juce::Point<float>(delay_x, bottom);
  else if (release_hover_)
    grab_point = juce::Point<float>(release_x, bottom);
  else if (sustain_hover_)
    grab_point = juce::Point<float>(decay_x, sustain_y);
  else if (attack_hover_)
    grab_point = juce::Point<float>(attack_x, top);
  else if (hold_hover_)
    grab_point = juce::Point<float>(hold_x, top);
  else if (attack_power_hover_)
    grab_point = juce::Point<float>((delay_x + attack_x) / 2.0f, 1.0f - 2.0f * padY(getSliderAttackValue(0.5f)) / height);
  else if (decay_power_hover_)
    grab_point = juce::Point<float>((hold_x + decay_x) / 2.0f, 1.0f - 2.0f * padY(getSliderDecayValue(0.5f)) / height);
  else if (release_power_hover_)
    grab_point = juce::Point<float>((decay_x + release_x) / 2.0f, 1.0f - 2.0f * padY(getSliderReleaseValue(0.5f)) / height);
...
void EnvelopeEditor::setTimePositions() {
  ...
  juce::Colour big_color = background_color_.overlaidWith(lighten);
  juce::Colour little_color = background_color_.overlaidWith(lighten.withMultipliedAlpha(t * t));
  int index = 1;
  for (; index * little_time_chunk < window_time_ && index < kMaxTimesShown; ++index) {
    ...
    juce::String display = formatTime(time);
...
//    return;
  ...
  if (y > -1.0f)
    ...
    float release = release_slider_->getValue();//getOutputsTotal(release_outputs_, release_slider_->getValue())[index];
    release = bitklavier::utils::max(release, 0.0f);
    current_position_alpha_ = current_position_alpha_ * std::min(kMinPositionAlphaDecay, release); //.set(index, current_position_alpha_[index] * std::min(kMinPositionAlphaDecay, release));
  }
  ...
  float current_phase = 1.0f; //envelope_phase_->value();//[index];
  if (current_phase <= bitklavier::kVoiceKill && current_phase >= bitklavier::kVoiceOn) {
    float width = getWidth();
    float height = getHeight();
    float marker_width = size_ratio_ * 2.0f * kMarkerWidth / width;
    float marker_height = size_ratio_ * 2.0f * kMarkerWidth / height;
  ...
  juce::Colour alt_color = color.interpolatedWith(background_color_, kCenterFade);
