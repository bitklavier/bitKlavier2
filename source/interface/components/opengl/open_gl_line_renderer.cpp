...
void OpenGlLineRenderer::init(OpenGlWrapper& open_gl) {
  ...
  open_gl.context.extensions.glBindBuffer(juce::gl::GL_ARRAY_BUFFER, line_buffer_);

  GLsizeiptr line_vert_size = static_cast<GLsizeiptr>(num_line_floats_ * sizeof(float));
  open_gl.context.extensions.glBufferData(juce::gl::GL_ARRAY_BUFFER, line_vert_size, line_data_.get(), juce::gl::GL_STATIC_DRAW);

  open_gl.context.extensions.glGenBuffers(1, &fill_buffer_);
  open_gl.context.extensions.glBindBuffer(juce::gl::GL_ARRAY_BUFFER, fill_buffer_);

  GLsizeiptr fill_vert_size = static_cast<GLsizeiptr>(num_fill_floats_ * sizeof(float));
  open_gl.context.extensions.glBufferData(juce::gl::GL_ARRAY_BUFFER, fill_vert_size, fill_data_.get(), juce::gl::GL_STATIC_DRAW);
  ...
  open_gl.context.extensions.glBufferData(juce::gl::GL_ELEMENT_ARRAY_BUFFER, line_size, indices_data_.get(), juce::gl::GL_STATIC_DRAW);
...
void OpenGlLineRenderer::drawLines(OpenGlWrapper& open_gl, bool left) {
  ...
  if (fill_) {
    ...
    open_gl.context.extensions.glVertexAttribPointer(fill_position_->attributeID, kFillFloatsPerVertex, juce::gl::GL_FLOAT,
                                                     juce::gl::GL_FALSE, kFillFloatsPerVertex * sizeof(float), nullptr);
    open_gl.context.extensions.glEnableVertexAttribArray(fill_position_->attributeID);
    juce::gl::glDrawElements(juce::gl::GL_TRIANGLE_STRIP, num_fill_vertices_, juce::gl::GL_UNSIGNED_INT, nullptr);
  }

  open_gl.context.extensions.glBindBuffer(juce::gl::GL_ARRAY_BUFFER, line_buffer_);
  shader_->use();
  open_gl.context.extensions.glVertexAttribPointer(position_->attributeID, kLineFloatsPerVertex, juce::gl::GL_FLOAT,
                                                  juce::gl::GL_FALSE, kLineFloatsPerVertex * sizeof(float), nullptr);
  ...
  juce::gl::glDrawElements(juce::gl::GL_TRIANGLE_STRIP, num_line_vertices_, juce::gl::GL_UNSIGNED_INT, nullptr);
