...
SynthSlider::SynthSlider(juce::String name, const juce::ValueTree &_vt) : OpenGlSlider(name),
                                                                          ...
                                                                          bypass_modulation_(false),
                                                                          modulation_bar_right_(true),
                                                                          ...
                                                                          shift_index_amount_(0),
                                                                          shift_is_multiplicative_(false),
                                                                          mouse_wheel_index_movement_(1.0),
                                                                          text_entry_width_percent_(
                                                                              kDefaultTextEntryWidthPercent),
                                                                          text_entry_height_percent_(
                                                                              kDefaultTextEntryHeightPercent),
                                                                          display_multiply_(0.0f),
                                                                          display_exponential_base_(2.0f),
                                                                          string_lookup_(nullptr),
                                                                          extra_modulation_target_(nullptr),
...
void SynthSlider::mouseDown(const juce::MouseEvent &e) {
    ...
    if (e.mods.isPopupMenu()) {
        ...
        parent_->showPopupSelector(this, e.getPosition(), options,
                                   [=](int selection, int) { handlePopupResult(selection); });
    } else {
        if (isRotary())
...
void SynthSlider::valueTreePropertyChanged(juce::ValueTree &treeWhosePropertyHasChanged,
    ...
    if (treeWhosePropertyHasChanged.hasType(IDs::MODULATABLE_PARAM)) {
        ...
        this->setNormalisableRange(juce::NormalisableRange<double>{
            attachment->getParameter()->range.start,
            attachment->getParameter()->range.end,
            attachment->getParameter()->range.interval,
...
double SynthSlider::getValueFromText(const juce::String &text) {
    ...
    if (attachment) {
        if (!attachment->getParameter()->getModParam().isValid()) {
            jassertfalse;
            // if you hit this then for some reason you have missed the adding of a valuetree to the chowdsp::parameter
            // the valuetree is the modulatable_param variable in the chowdsp::floatparameter and it MUST be set.
            // It is typically added/set in the pluginbase constructor. from the setupModulationMappings() function
            // as it stands all sliders that can be reranged can be modulated. if that changes this code may have to change as well

            // in the event that you hit this you should still return a valid value but it should still be clamped
            // the jassertfalse will only trigger in debug so this should still safely just do nothing in the event that there is a problem
            return val;
        }
        ...
        if (attachment->getParameter()->range.end < unclamped_val) {
            ...
            attachment->getParameter()->range = juce::NormalisableRange<float>{
                range.start, static_cast<float>(unclamped_val), range.interval, range.skew
            };
            ...
            attachment->getParameter()->range = juce::NormalisableRange<float>{
                static_cast<float>(unclamped_val), range.start, range.interval, range.skew
            };
        }
        val = unclamped_val;
        attachment->setNormalisableRange(attachment->getParameter()->range);
        this->setNormalisableRange(juce::NormalisableRange<double>{
            attachment->getParameter()->range.start, attachment->getParameter()->range.end,
            attachment->getParameter()->range.interval, attachment->getParameter()->range.skew
        });
    }
...
void SynthSlider::drawRotaryShadow(juce::Graphics &g) {
    ...
    if (body_radius >= 0.0f && body_radius < width) {
        ...
        g.setColour(body);
        juce::Rectangle<float> ellipse(center_x - body_radius, center_y - body_radius, 2.0f * body_radius,
                                       2.0f * body_radius);
    ...
    shadow_outline.addCentredArc(center_x, center_y, radius, radius, 0, -kRotaryAngle, kRotaryAngle, true);
...
juce::Rectangle<int> SynthSlider::getModulationMeterBounds() const {
    ...
    if (getSliderStyle() == LinearBar) {
        return juce::Rectangle<int>(mod_bounds.getX() + buffer, mod_bounds.getY(), mod_bounds.getWidth() - 2 * buffer,
                                    mod_bounds.getHeight());
    }
    return juce::Rectangle<int>(mod_bounds.getX(), mod_bounds.getY() + buffer, mod_bounds.getWidth(),
                                mod_bounds.getHeight() - 2 * buffer);
}
...
void SynthSlider::setRotaryTextEntryBounds() {
    ...
    text_entry_->setBounds((getWidth() - text_width) / 2, (getHeight() - text_height + 1) / 2 + y_offset, text_width,
                           text_height);
}
