...
SynthSlider::SynthSlider (juce::String name, const juce::ValueTree& _vt) : OpenGlSlider (name), show_popup_on_hover_ (false), scroll_enabled_ (true), bipolar_modulation_ (false), stereo_modulation_ (false), bypass_modulation_ (false), modulation_bar_right_ (true), snap_to_value_ (false), hovering_ (false), has_parameter_assignment_ (false), use_suffix_ (true), snap_value_ (0.0), text_height_percentage_ (0.0f), sensitivity_ (kDefaultSensitivity), popup_placement_ (juce::BubbleComponent::below), modulation_control_placement_ (juce::BubbleComponent::below), max_display_characters_ (kDefaultFormatLength), max_decimal_places_ (kDefaultFormatDecimalPlaces), shift_index_amount_ (0), shift_is_multiplicative_ (false), mouse_wheel_index_movement_ (1.0), text_entry_width_percent_ (kDefaultTextEntryWidthPercent), text_entry_height_percent_ (kDefaultTextEntryHeightPercent), display_multiply_ (0.0f), display_exponential_base_ (2.0f), string_lookup_ (nullptr), extra_modulation_target_ (nullptr), synth_interface_ (nullptr), vt (_vt) /*attachment(param,pluginState, *this)*/
{
...
void SynthSlider::drawRotaryShadow (juce::Graphics& g)
{
    ...
    if ((!findColour (Skin::kRotaryArcUnselected, true).isTransparent() && isActive()) || (!findColour (Skin::kRotaryArcUnselectedDisabled, true).isTransparent() && !isActive()))
    {
...
juce::Rectangle<int> SynthSlider::getModulationMeterBounds() const
{
    ...
    {
        return juce::Rectangle<int> (mod_bounds.getX() + buffer, mod_bounds.getY(), mod_bounds.getWidth() - 2 * buffer, mod_bounds.getHeight());
    }
    return juce::Rectangle<int> (mod_bounds.getX(), mod_bounds.getY() + buffer, mod_bounds.getWidth(), mod_bounds.getHeight() - 2 * buffer);
}
...
void SynthSlider::setRotaryTextEntryBounds()
{
    ...
    text_entry_->setBounds ((getWidth() - text_width) / 2, (getHeight() - text_height + 1) / 2 + y_offset, text_width, text_height);
}
