...
{
    bool soloed    = nparams_.soloed_.load(std::memory_order_relaxed);
    bool userMuted = nparams_.userMuted_.load(std::memory_order_relaxed);
    bool soloMuted = nparams_.soloMuted_.load(std::memory_order_relaxed);
...
void NostalgicParametersView::resized()
{
    ...
    juce::Rectangle<int> middleRow = bounds.removeFromBottom (Skin::kKnobSectionHeight*10.5 + largepadding);
