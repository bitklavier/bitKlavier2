...
ModulationLineView::ModulationLineView(ConstructionSite &site, juce::UndoManager &um, SynthGuiData *data)
    : SynthSection("modlineview"), site(site),
      ...
      current_source_(nullptr), undoManager(um), connection_list(data->synth->getActiveModConnectionList()) {
...
void ModulationLineView::modulationDropped(const juce::ValueTree &source, const juce::ValueTree &destination) {
    ...
    _connection.setProperty(IDs::src, juce::VariantConverter<juce::AudioProcessorGraph::NodeID>::toVar(sourceId),
                            nullptr);
    _connection.setProperty(IDs::dest, juce::VariantConverter<juce::AudioProcessorGraph::NodeID>::toVar(destId),
                            nullptr);
