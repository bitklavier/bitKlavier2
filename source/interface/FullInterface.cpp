...
FullInterface::FullInterface(SynthGuiData *synth_data) : SynthSection("full_interface"), width_(0), resized_width_(0),
    ...
    vt = t;
    main_ = std::make_unique<MainSection>(synth_data->tree.getChildWithName(IDs::PIANO), synth_data->um, open_gl_,
                                          synth_data);
...
void FullInterface::resized() {
    ...
    prep_popup->setBounds(100, 100, new_bounds.getWidth() / (1.25 * display_scale_),
                          new_bounds.getHeight() / (1.25 * display_scale_));
...
void FullInterface::newOpenGLContextCreated() {
    ...
    if (unsupported_) {
        juce::NativeMessageBox::showMessageBoxAsync(juce::AlertWindow::WarningIcon, "Unsupported OpenGl Version",
                                                    juce::String("Vial requires OpenGL version: ") + juce::String(
                                                        kMinOpenGlVersion) +
                                                    juce::String("\nSupported version: ") + juce::String(
                                                        version_supported));
        return;
    }
