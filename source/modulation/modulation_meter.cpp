...
juce::Rectangle<float> ModulationMeter::getMeterBounds() {
  ...
  if (!destination_->isRotary() && !destination_->isTextOrCurve()) {
    ...
    if (destination_->isHorizontal())
      return juce::Rectangle<float>(0.0f, outer_area + border, width, inner_area - outer_area - 2.0f * border);
    return juce::Rectangle<float>(outer_area + border, 0.0f, inner_area - outer_area - 2.0f * border, height);
  }
...
void ModulationMeter::setAmountQuadVertices(OpenGlQuad& quad) {
  ...
  bool vertical_bar = destination_->getSliderStyle() == juce::Slider::LinearBarVertical || destination_->isTextOrCurve();
  if (vertical_bar)
