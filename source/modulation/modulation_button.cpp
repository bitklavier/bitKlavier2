...
ModulationButton:: ModulationButton(juce::String name) : PlainShapeComponent(name), parent_(nullptr),
  ...
  setColor(juce::Colours::transparentWhite);
}
...
juce::Rectangle<int> ModulationButton::getModulationAmountBounds(int index, int total) {
  int columns = kModulationKnobColumns;

  int row = index / columns;
  int column = index % columns;

  juce::Rectangle<int> all_bounds = getModulationAreaBounds();
...
juce::Rectangle<int> ModulationButton::getMeterBounds() {
  ...
  return juce::Rectangle<int>(1, 1, meter_width, meter_height);
}

juce::Rectangle<int> ModulationButton::getModulationAreaBounds() {
  ...
  return juce::Rectangle<int>(widget_x, widget_y, widget_width, widget_height);
}

void ModulationButton::paintBackground(juce::Graphics& g) {
  ...
  juce::Rectangle<float> meter_bounds = getMeterBounds().toFloat();
  int width = getWidth();
  int adjusted_width = connect_right_ ? width * 2 : width;
  juce::Rectangle<float> bounds(0, 0, adjusted_width, getHeight());
  ...
  juce::String text = text_override_;
//  if (text.isEmpty())
//    text = ModulationMatrix::getUiSourceDisplayName(getName());
  ...
  g.drawText(text, meter_bounds.getRight(), 0, width - meter_bounds.getRight(),
             font_area_height, juce::Justification::centred);

  if (connect_right_ && !selected_) {
    int shadow_width = width * kShadowArea;
    juce::Colour shadow_color = findColour(Skin::kShadow, true);
    juce::ColourGradient gradient(shadow_color, width, 0, shadow_color.withAlpha(0.0f), width - shadow_width, 0, false);
...
void ModulationButton::mouseDrag(const juce::MouseEvent& e) {
...
void ModulationButton::mouseEnter(const juce::MouseEvent& e) {
  mouse_state_ = kHover;
...
void ModulationButton::mouseExit(const juce::MouseEvent& e) {
  mouse_state_ = kNone;
  show_drag_drop_ = false;
}

void ModulationButton::mouseWheelMove(const juce::MouseEvent& e, const juce::MouseWheelDetails& wheel) {
  for (Listener* listener : listeners_)
...
void ModulationButton::disconnectModulation(bitklavier::ModulationConnection* connection) {
