...
void BarRenderer::init(OpenGlWrapper& open_gl) {
  ...
  open_gl.context.extensions.glBindBuffer(juce::gl::GL_ARRAY_BUFFER, bar_buffer_);

  GLsizeiptr vert_size = static_cast<GLsizeiptr>(kFloatsPerBar * total_points_ * sizeof(float));
  open_gl.context.extensions.glBufferData(juce::gl::GL_ARRAY_BUFFER, vert_size, bar_data_.get(), juce::gl::GL_STATIC_DRAW);

  open_gl.context.extensions.glGenBuffers(1, &bar_corner_buffer_);
  open_gl.context.extensions.glBindBuffer(juce::gl::GL_ARRAY_BUFFER, bar_corner_buffer_);

  GLsizeiptr corner_size = static_cast<GLsizeiptr>(kCornerFloatsPerBar * total_points_ * sizeof(float));
  open_gl.context.extensions.glBufferData(juce::gl::GL_ARRAY_BUFFER, corner_size, bar_corner_data_.get(), juce::gl::GL_STATIC_DRAW);

  open_gl.context.extensions.glGenBuffers(1, &bar_indices_buffer_);
  open_gl.context.extensions.glBindBuffer(juce::gl::GL_ELEMENT_ARRAY_BUFFER, bar_indices_buffer_);

  GLsizeiptr bar_size = static_cast<GLsizeiptr>(kTriangleIndicesPerBar * total_points_ * sizeof(int));
  open_gl.context.extensions.glBufferData(juce::gl::GL_ELEMENT_ARRAY_BUFFER, bar_size, bar_indices_.get(), juce::gl::GL_STATIC_DRAW);
...
void BarRenderer::drawBars(OpenGlWrapper& open_gl) {
  ...
  open_gl.context.extensions.glVertexAttribPointer(position_->attributeID, kFloatsPerVertex, juce::gl::GL_FLOAT,
                                                   juce::gl::GL_FALSE, kFloatsPerVertex * sizeof(float), nullptr);
  open_gl.context.extensions.glEnableVertexAttribArray(position_->attributeID);

  open_gl.context.extensions.glBindBuffer(juce::gl::GL_ARRAY_BUFFER, bar_corner_buffer_);
  open_gl.context.extensions.glVertexAttribPointer(corner_->attributeID, kCornerFloatsPerVertex, juce::gl::GL_FLOAT,
                                                  juce::gl::GL_FALSE, kCornerFloatsPerVertex * sizeof(float), nullptr);
  open_gl.context.extensions.glEnableVertexAttribArray(corner_->attributeID);

  juce::gl::glDrawElements(juce::gl::GL_TRIANGLES, kTriangleIndicesPerBar * total_points_, juce::gl::GL_UNSIGNED_INT, nullptr);
