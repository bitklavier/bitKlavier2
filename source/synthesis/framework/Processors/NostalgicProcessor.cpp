 ...
 */
juce::Array<float> NostalgicProcessor::getMidiNoteTranspositions()
{
...
void NostalgicProcessor::processBlock (juce::AudioBuffer<float>& buffer, juce::MidiBuffer& midiMessages)
{
     ...
     *
     *  I'm not sure we have any of these for Nostalgic, but no harm in calling it, and for reference going forward
     */
    ...
    juce::Array<float> updatedTransps = getMidiNoteTranspositions(); // from the Nostalgic transposition slider
     ...
     * we look at directBusLayout() in NostalgicProcessor.h, and see that 'Send' is the 3rd output, so its busIndex is 2
...
/**
 * NostalgicProcessor::processBlockBypassed is called when this Nostalgic prep is NOT in the active,
