...
TuningProcessor::TuningProcessor (SynthBase& parent, const juce::ValueTree& vt, juce::UndoManager* um ) : PluginBase (parent, vt, um, tuningBusLayout())
{
    ...
    // initialize offsets
    setOffsetsFromTuningSystem(state.params.tuningState.tuningSystem->get(), state.params.tuningState.fundamental->getIndex(), state.params.tuningState.circularTuningOffset, state.params.tuningState.circularTuningOffset_custom);
