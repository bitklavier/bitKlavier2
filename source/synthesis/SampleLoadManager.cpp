...
static inline juce::String sfzBaseFromKey(const juce::String &key) {
    return key.upToFirstOccurrenceOf("||", false, false);
}

static inline juce::String sfzPresetFromKey(const juce::String &key) {
    return key.fromFirstOccurrenceOf("||", false, false);
}

static inline juce::String makeSFZKey(const juce::String &base, const juce::String &preset) {
    if (preset.isEmpty())
        return base;
    return base + "||" + preset;
}
...
SampleLoadManager::SampleLoadManager(SynthBase *parent,
                                     std::shared_ptr<UserPreferencesWrapper> preferences) : preferences(preferences),
...
juce::BigInteger SampleLoadManager::getMidiRange(juce::String pitchName) {
    ...
    if (pitchIndex >= allKeysWithSamples.size() - 1)
        ...
        gapUp = std::trunc(((upperNeighbor - pitchNum) / 2.) - 0.1);
        // subtract something small, so it truncates down when even, but still works when odd...
    }
...
void SampleLoadManager::loadSamples_sub(bitklavier::utils::BKPianoSampleType thisSampleType, std::string name) {
    ...
    progressPtr->totalJobs++; // one job per sample type
    samplerSoundset[soundsetName] = new juce::ReferenceCountedArray<BKSynthesiserSound>();
    sampleLoader.addJob(new SampleLoadJob(
                            audioFormatManager.get(),
...
bool SampleLoadManager::sfzHasPresets(const juce::String &sfzName) const {
    if (auto *sfz = findSFZSoundByName(sfzName))
        return sfz->num_subsounds() > 0;
...
bool SampleLoadManager::changeSFZPresetAndUpdateTree(const juce::String &currentSfzKey,
    ...
    SFZSound &sound = *it->second;

    const int numSubs = sound.num_subsounds();
    if (newPresetIndex < 0 || newPresetIndex >= numSubs)
        return false;
    const juce::String newPresetName = juce::String(
        sound.subsound_name(newPresetIndex)
    );

    const juce::String newKey = makeSFZKey(base, newPresetName);
    ...
    // Switch subsound
    sound.use_subsound(newPresetIndex);
    ...
    if (samplerSoundset.contains(oldKeyStd)) {
        ...
        for (int i = 0; i < sound.num_regions(); i++) {
            ...
            // --- Add to soundset ---
            samplerSoundset[newKeyStd]->add(newSound);

            // --- Debug output ---
            DBG("Loaded region " + juce::String(i) +
                ": " + region->sample->short_name() +
    ...
    // Re-key sfzBanks entry
    // auto ptr = std::move();
    sfzBanks.emplace(newKeyStd, std::move(it->second));
    ...
    // // Now remove old samplerSoundset entry
    // if (samplerSoundset.contains(oldKeyStd))
    //     samplerSoundset.erase(oldKeyStd);
    // // now remove old soundfont
    // sfzBanks.erase(it);
    return true;
}
