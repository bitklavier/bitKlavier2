...
void SynthBase::valueTreePropertyChanged(juce::ValueTree &treeWhosePropertyHasChanged,
    ...
    if (property == IDs::isActive && treeWhosePropertyHasChanged.hasType(IDs::PIANO) && static_cast<int>(
            treeWhosePropertyHasChanged.getProperty(IDs::isActive)) == 1) {
        if (getGuiInterface()) {
...
void SynthBase::addTuningConnection(juce::AudioProcessorGraph::NodeID src, juce::AudioProcessorGraph::NodeID dest) {
    ...
    dynamic_cast<bitklavier::InternalProcessor *>(destNode->getProcessor())->setTuning(
        dynamic_cast<TuningProcessor *>(sourceNode->getProcessor()));
}
...
void SynthBase::connectModulation(bitklavier::ModulationConnection *connection) {
    ...
    auto mod_src = tree.getChildWithName(IDs::PIANO).getChildWithName(IDs::PREPARATIONS).getChildWithProperty(
        IDs::uuid, juce::String(uuid));
    ...
    auto mod_dst = tree.getChildWithName(IDs::PIANO).getChildWithName(IDs::PREPARATIONS).getChildWithProperty(
        IDs::uuid, juce::String(dst_uuid));
...
bool SynthBase::connectReset(const juce::ValueTree &v) {
    ...
    auto source_index = engine_->getNodeForId(sourceId)->getProcessor()->
            getChannelIndexInProcessBlockBuffer(false, 2, 0); //2 is reset
