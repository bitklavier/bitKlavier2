...
## Plugin state restore + standalone/plugin isolation (fixed Jun 13 2026)

See `memory/plugin_state_restore.md`. Five compounding bugs in Logic AU restore: (1) `prepareToPlay` can fire before `setStateInformation` — queued lambdas must re-check at fire time, not just at queue time; (2) `loadFromFile` immediate path must clear any stale `pendingPresetTree` from a still-in-flight deferred load or `finishedSampleLoading` later overwrites; (3) `Cable::connectionAdded` routes setVisible through `executeOnGLThread` which silently no-ops if GL not yet attached → defer editor-ctor `setActivePiano` via callAsync; (4) `getStateInformation` was reading `user_prefs->tree.last_gallery_path` which is a `juce::SharedResourcePointer` singleton — all tracks wrote the same path; per-track save must use `getActiveFile()`; (5) `SynthBase::loadFromFile`/`saveToFile` wrote `user_prefs` regardless of plugin/standalone — gated behind `virtual bool isStandaloneApp() const` (overridden in `SynthEditor`). Also: `samplesLoaded` is now authoritative only after `finishedSampleLoading`, since `loadSamples` returns true on its "already loading" branch. The Logic "Audio Unit reported a problem" instability warning on Prelude 1 was a symptom of this restore race, not a separate bug.
