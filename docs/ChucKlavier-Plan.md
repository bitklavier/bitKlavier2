...
- **RT-safety audit result (completed):** One confirmed per-sample heap operation: `std::map<Chuck_VM_Shred*,Chuck_VM_Shred*>::erase()` in `Chuck_VM_Shreduler::remove_blocked()` (chuck_vm.cpp:2672), called once per sample as a permanently-looping shred re-awakens. All other paths (event buffers, global queue, UGen ticks) are clean in the empty-queue, permanent-loop steady state. Verdict: **proceed with AT approach** — consistent with Chunity/miniAudicle usage; on Apple Silicon the per-sample `std::map` node deallocation is sub-microsecond and deterministic. If audio glitches are observed in practice, fall back to worker-thread + lock-free ring buffer.
