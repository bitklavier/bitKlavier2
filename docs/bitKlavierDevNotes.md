...
---------
## Action Sequences
would be great to have some of these detailed, to help in creating new preps/params and debugging:
- When "Save" is initiated, here's what happens:
  - ...
- When "Load" is initiated:
- When a slider is changed by the user:
- When a mod is triggered:
- When a note is played on the keyboard:
...
- [x] for the serializer/deserializers in TuningProcessor.cpp, take a look at my todo questions in the .cpp file
...
- [x] check on saving/loading galleries and Direct, not working well right now (drawing funny, Direct preps not loading)
  - Myra is working on. but Direct saves now, except Transpositions, so check that
- [x] check on Mods with Direct; are they working for all params? do they save?
